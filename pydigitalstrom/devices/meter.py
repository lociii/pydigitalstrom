...
class DSMeter(DSDevice):
    ...
    async def update(self):
        ...
        """
        # update meta data
        data = await self.request(url=self._client.URL_METERS)
        if 'circuits' in data:
        ...
        data = await self.request(url=self.URL_POWER_CONSUMPTION, id=self._id)
        if 'consumption' in data:
            self._current_consumption = data['consumption']

        # update overall consumption
        data = await self.request(url=self.URL_POWER_METER, id=self._id)
